pipeline {
    ...
    environment {
        ...
        DOCKER_REGISTRY = '192.168.1.104:5000'  // Địa chỉ registry riêng
        ...
        DEPLOYMENT_HOST = '192.168.1.104'  // Thay bằng IP máy deployment của bạn
                    ...
                    if (isUnix()) {
                        sh '''
                            ...
                            npx prisma migrate diff \\
                                ...
                                --script 2>&1 | grep -v "\\[dotenv" | grep -v "^Loaded Prisma" | grep -v "^Prisma config" | grep -v "^warn" | grep -v "For more information" | grep -v "^  [A-Z]" | grep -v "^$" > prisma/test-schema.sql
